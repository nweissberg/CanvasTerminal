import * as THREE from '../__3D__/three.module.js';
...
function loadScene(three_scene, blender_file, cache) {
    ...
    // console.log(blender_file)
    const world = blender_file.objects.World[0]
    var horizon_color = color(world.horr,world.horg,world.horb,1)
    if(three_scene!=undefined) three_scene.scene.background = horizon_color
    var blend_objects = {"lights":{},"meshes":{}}

	for (let i = 0; i < blender_file.objects.Object.length; i++) {

		let obj = blender_file.objects.Object[i];
        // if(obj.sensors.first != undefined) console.log(obj.data.aname, obj.sensors.last, obj.sensors.first.type)
		...
		if (obj.type == blender_types.lamp) {
            ...
            if(light.name == "" || light.name == undefined){
                light.name = "lamp_"+(Object.keys(blend_objects.lights).length+1).toString()
            }
            blend_objects.lights[light.name] = light
			if(three_scene!=undefined) three_scene.scene.add(light);
		}
		...
		if (obj.type == blender_types.mesh_object) {
			if (obj.data) {
				...
				if(three_scene!=undefined) three_scene.scene.add(mesh);
                ...
                if(mesh.name == "" || mesh.name == undefined){
                    mesh.name = "mesh_"+(Object.keys(blend_objects.meshes).length+1).toString()
                }

                blend_objects.meshes[obj.data.id.name] = mesh
                // console.log(mesh)
				//*/
			}
		}
...
module.exports = (() => {
    ...
    function applyTexture(blender_texture, material) {
        ...
        if (blender_texture && blender_texture.tex && blender_texture.tex.ima) {
            ...
            three_texture.wrapS = three_texture.wrapT = THREE.RepeatWrapping;
            // three_texture.offset.set( 0, 0 );
            // three_texture.repeat.set( 2, 2 );
    ...
    return function createThreeJSMaterial(blend_mat) {
        ...
        var e = blend_mat.emit
        var emissive = new THREE.Color(e*blend_mat.r,e*blend_mat.g,e*blend_mat.b)
        // for(var key in blend_mat){
        //     console.log(blend_mat[key])
        // }
        // var matVars = ["adapt_thresh_mir:"+blend_mat.adapt_thresh_mir,"adapt_thresh_tra:"+blend_mat.adapt_thresh_tra,"add:"+blend_mat.add,"adt:"+blend_mat.adt,"alpha:"+blend_mat.alpha,"amb:"+blend_mat.amb,"ambb:"+blend_mat.ambb,"ambg:"+blend_mat.ambg,"ambr:"+blend_mat.ambr,"ang:"+blend_mat.ang,"aniso_gloss_mir:"+blend_mat.aniso_gloss_mir,"b:"+blend_mat.b,"darkness:"+blend_mat.darkness,"diff_shader:"+blend_mat.diff_shader,"dist_mir:"+blend_mat.dist_mir,"dynamode:"+blend_mat.dynamode,"emit:"+blend_mat.emit,"fadeto_mir:"+blend_mat.fadeto_mir,"fh:"+blend_mat.fh,"fhdist:"+blend_mat.fhdist,"filter:"+blend_mat.filter,"flag:"+blend_mat.flag,"flareboost:"+blend_mat.flareboost,"flarec:"+blend_mat.flarec,"flaresize:"+blend_mat.flaresize,"fresnel_mir:"+blend_mat.fresnel_mir,"fresnel_mir_i:"+blend_mat.fresnel_mir_i,"fresnel_tra:"+blend_mat.fresnel_tra,"fresnel_tra_i:"+blend_mat.fresnel_tra_i,"friction:"+blend_mat.friction,"g:"+blend_mat.g,"gloss_mir:"+blend_mat.gloss_mir,"gloss_tra:"+blend_mat.gloss_tra,"group:"+blend_mat.group,"har:"+blend_mat.har,"hasize:"+blend_mat.hasize,"index:"+blend_mat.index,"ipo:"+blend_mat.ipo,"lbias:"+blend_mat.lbias,"line_col:"+blend_mat.line_col,"line_priority:"+blend_mat.line_priority,"linec:"+blend_mat.linec,"mapflag:"+blend_mat.mapflag,"mapto:"+blend_mat.mapto,"mapto_textured:"+blend_mat.mapto_textured,"material_type:"+blend_mat.material_type,"mirb:"+blend_mat.mirb,"mirg:"+blend_mat.mirg,"mirr:"+blend_mat.mirr,"ml_flag:"+blend_mat.ml_flag,"mode:"+blend_mat.mode,"mode2:"+blend_mat.mode2,"mode2_l:"+blend_mat.mode2_l,"mode_l:"+blend_mat.mode_l,"mtex:"+blend_mat.mtex,"nmap_tangent_names:"+blend_mat.nmap_tangent_names,"nmap_tangent_names_count:"+blend_mat.nmap_tangent_names_count,"nodetree:"+blend_mat.nodetree,"pad:"+blend_mat.pad,"pad2:"+blend_mat.pad2,"pad3:"+blend_mat.pad3,"pad4:"+blend_mat.pad4,"pad5:"+blend_mat.pad5,"paint_active_slot:"+blend_mat.paint_active_slot,"paint_clone_slot:"+blend_mat.paint_clone_slot,"param:"+blend_mat.param,"pr_lamp:"+blend_mat.pr_lamp,"pr_texture:"+blend_mat.pr_texture,"pr_type:"+blend_mat.pr_type,"preview:"+blend_mat.preview,"r:"+blend_mat.r,"ramp_col:"+blend_mat.ramp_col,"ramp_show:"+blend_mat.ramp_show,"ramp_spec:"+blend_mat.ramp_spec,"rampblend_col:"+blend_mat.rampblend_col,"rampblend_spec:"+blend_mat.rampblend_spec,"rampfac_col:"+blend_mat.rampfac_col,"rampfac_spec:"+blend_mat.rampfac_spec,"rampin_col:"+blend_mat.rampin_col,"rampin_spec:"+blend_mat.rampin_spec,"ray_depth:"+blend_mat.ray_depth,"ray_depth_tra:"+blend_mat.ray_depth_tra,"ray_mirror:"+blend_mat.ray_mirror,"ref:"+blend_mat.ref,"reflect:"+blend_mat.reflect,"refrac:"+blend_mat.refrac,"rgbsel:"+blend_mat.rgbsel,"ringc:"+blend_mat.ringc,"rms:"+blend_mat.rms,"roughness:"+blend_mat.roughness,"samp_gloss_mir:"+blend_mat.samp_gloss_mir,"samp_gloss_tra:"+blend_mat.samp_gloss_tra,"sbias:"+blend_mat.sbias,"seed1:"+blend_mat.seed1,"seed2:"+blend_mat.seed2,"septex:"+blend_mat.septex,"shad_alpha:"+blend_mat.shad_alpha,"shade_flag:"+blend_mat.shade_flag,"shadowonly_flag:"+blend_mat.shadowonly_flag,"spec:"+blend_mat.spec,"spec_shader:"+blend_mat.spec_shader,"specb:"+blend_mat.specb,"specg:"+blend_mat.specg,"specr:"+blend_mat.specr,"spectra:"+blend_mat.spectra,"sss_back:"+blend_mat.sss_back,"sss_col:"+blend_mat.sss_col,"sss_colfac:"+blend_mat.sss_colfac,"sss_error:"+blend_mat.sss_error,"sss_flag:"+blend_mat.sss_flag,"sss_front:"+blend_mat.sss_front,"sss_ior:"+blend_mat.sss_ior,"sss_preset:"+blend_mat.sss_preset,"sss_radius:"+blend_mat.sss_radius,"sss_scale:"+blend_mat.sss_scale,"sss_texfac:"+blend_mat.sss_texfac,"starc:"+blend_mat.starc,"strand_ease:"+blend_mat.strand_ease,"strand_end:"+blend_mat.strand_end,"strand_min:"+blend_mat.strand_min,"strand_sta:"+blend_mat.strand_sta,"strand_surfnor:"+blend_mat.strand_surfnor,"strand_uvname:"+blend_mat.strand_uvname,"strand_widthfade:"+blend_mat.strand_widthfade,"subsize:"+blend_mat.subsize,"texact:"+blend_mat.texact,"texco:"+blend_mat.texco,"texpaintslot:"+blend_mat.texpaintslot,"tot_slots:"+blend_mat.tot_slots,"translucency:"+blend_mat.translucency,"tx_falloff:"+blend_mat.tx_falloff,"tx_limit:"+blend_mat.tx_limit,"use_nodes:"+blend_mat.use_nodes,"vcol_alpha:"+blend_mat.vcol_alpha,"xyfrict:"+blend_mat.xyfrict,"zoffs:"+blend_mat.zoffs]
        // console.log(matVars.join().replaceAll(",", "\n"))
...
module.exports = function createThreeJSBufferGeometry(blender_mesh, origin) {
    //get materials
    // console.log(blender_mesh.id.name)
    let pick_material = 0,
        mesh = blender_mesh,
        faces = mesh.mpoly,
        loops = mesh.mloop,
    ...
    geometry.name = blender_mesh.id.name
