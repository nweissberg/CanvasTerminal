...
var retry_bot = new bttn2D({
  ...
  onTch_U:(function(){
    // reset_game()
    if(high_score_obj)high_score_obj.pos.y = 20
    game_hud.tab = 0
    game_stg.tab = 1
  })
})
...
var char = new pln2D({
  ...
  loop:(function(){
    ...
    if(getKey(['UP','W'],1) && this.last_dir != 3) this.dir = 0
    if(getKey(['LEFT','A'],1) && this.last_dir != 2) this.dir = 1
    if(getKey(['RIGHT','D'],1) && this.last_dir != 1) this.dir = 2
    if(getKey(['DOWN','S'],1) && this.last_dir != 0) this.dir = 3
      ...
      if(tail.length == 0){
        ...
        score.lerpTo("pos.y",-10,15).exec((function(){
          ...
          if(points > high_score.points){
            game_hud.tab = 2 
            high_score_obj.pos.y = 0
          // saveVar("high_score",points)
          }
    ...
    var max_w = Math.floor(stg_w/grid)*grid
    var max_h = Math.floor(stg_h/grid)*grid
